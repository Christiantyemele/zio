...
import java.util.concurrent.{ThreadLocalRandom => JThreadLocalRandom}
...
private final class NioScheduler(autoBlocking: Boolean) extends Executor { parent =>
  ...
  private[this] val state          = new AtomicInteger(poolSize << 16) // active (hi 16) | searching (lo 16)
  ...
  private[this] def makeSupervisor(): Thread =
    new Thread {
      private[this] val previousOpCounts   = Array.fill(poolSize)(-1L)
      private[this] val unchangedIntervals = Array.fill(poolSize)(0)
      ...
      override def run(): Unit =
        while (!isInterrupted) {
          ...
          while (workerId < poolSize) {
            ...
            if (worker.active) {
              val currentOpCount  = worker.opCount
              val previousOpCount = previousOpCounts(workerId)
              if (currentOpCount == previousOpCount && worker.running.get() && worker.currentRunnable.isInstanceOf[FiberRunnable]) {
                val nextIntervals = unchangedIntervals(workerId) + 1
                unchangedIntervals(workerId) = nextIntervals
                // If the worker hasn't made progress for a few supervisor ticks, assume it is implicitly blocking.
                if (nextIntervals >= 3) {
                  unchangedIntervals(workerId) = 0
                  previousOpCounts(workerId) = -1L
                  if (worker.replaced.compareAndSet(false, true)) {
  ...
  private[this] def maybeUnparkWorker(currentState: Int): Unit = {
    ...
    if (currentActive != poolSize && currentSearching == 0) {
      var worker = idle.poll()
      while ((worker ne null) && worker.active) {
        worker = idle.poll()
      }
      if (worker ne null) {
  ...
  private[this] def makeWorker(index: Int): NioScheduler.Worker =
    new NioScheduler.Worker(index, autoBlocking) {
      override def submit(runnable: Runnable, transfer: Runnable => Unit): Unit = {
        inbox.offer(runnable)
        if (replaced.get() && inbox.remove(runnable)) {
          load.decrementAndGet()
          transfer(runnable)
        } else {
          // If this worker is parked, account for it becoming active again.
          if (!active) {
            active = true
            parent.state.getAndAdd(0x10000)
          }
      ...
      override def run(): Unit =
        try {
          var continue = true
          val random   = JThreadLocalRandom.current()
          var searching = false
          while (continue && !isInterrupted) {
            ...
            if (runnable eq null) {
              if (replaced.get()) {
                ...
                runnable = parent.globalQueue.poll(random)
                if ((runnable eq null) && !replaced.get()) {
                  if (!searching) {
                    val currentState  = parent.state.get
                    val currentActive = (currentState & 0xffff0000) >> 16
                    if (2 * currentActive < poolSize) {
                      parent.state.getAndIncrement()
                      searching = true
                    }
                  }
                  if (searching) {
                    ...
                    while (i < poolSize && loop && (runnable eq null) && !replaced.get()) {
                      val idx      = (i + offset) % poolSize
                      val candidate = parent.workers.get(idx)
                      if ((candidate ne this) && candidate.active) {
                        runnable = candidate.inbox.poll()
                      }
                      if (runnable ne null) loop = false
                      i += 1
                    }
                    if (runnable eq null) {
                      runnable = parent.globalQueue.poll(random)
                ...
                if ((runnable eq null) && !replaced.get()) {
                  val currentState =
                    if (searching) parent.state.addAndGet(0xfffeffff) // active--, searching--
                    else parent.state.addAndGet(0xffff0000) // active--
                  val currentSearching = currentState & 0xffff
                  active = false
                  parent.idle.offer(this)
                  if (currentSearching == 0 && searching) {
                    if (!parent.globalQueue.isEmpty()) {
                      val s = parent.state.get
                      parent.maybeUnparkWorker(s)
                    }
                  }
                  while (!active && !isInterrupted && !replaced.get()) {
                    LockSupport.park(this)
                  }
                  searching = false
                  runnable = inbox.poll()
                  if (runnable eq null) runnable = parent.globalQueue.poll(random)
                }
                if ((runnable eq null) && replaced.get()) {
                  drainTo(parent.transfer)
                  continue = false
            ...
            if (runnable ne null) {
              if (searching) {
                searching = false
                val currentState = parent.state.decrementAndGet() // searching--
                parent.maybeUnparkWorker(currentState)
              }
              running.set(true)
...
private object NioScheduler {
  ...
  private sealed abstract class Worker(val index: Int, autoBlocking: Boolean) extends Thread with BlockContext {
    ...
    def submit(runnable: Runnable, transfer: Runnable => Unit): Unit = {
      inbox.offer(runnable)
      if (replaced.get() && inbox.remove(runnable)) {
        load.decrementAndGet()
        transfer(runnable)
